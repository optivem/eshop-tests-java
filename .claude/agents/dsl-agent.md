...
7. Check whether any new or changed driver interfaces are in an `external/` package (e.g. `driver-port/.../external/clock`, `driver-port/.../external/erp`). Set a flag: **external system interfaces changed = yes/no**.
8. Report back: the full DSL implementation, all driver interface changes (method signatures, new DTOs), and the external system interfaces flag. Do NOT commit. Do NOT proceed to COMMIT. **STOP here and wait for human approval.**
